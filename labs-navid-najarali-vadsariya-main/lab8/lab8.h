...
public:
	...
	void copyHelper(const Node* node)
	{
		...
		if (node->left_ != nullptr) {
			copyHelper(node->left_);
		}
		if (node->right_ != nullptr) {
			copyHelper(node->right_);
		}
	}
	...
	void remove(const T &data)
	{
		if (root_ != nullptr)
		{
			...
			if(nodeToDelete != nullptr){
				bool isRoot = root_ == nodeToDelete;
				if( nodeToDelete->left_ == nullptr && nodeToDelete->right_ == nullptr){
					...
					if (isRoot){
						...
						if (nodeToDelete && nodeToDelete->left_ != nullptr) {
							leftChild = nodeToDelete->left_;
						}
						if(leftChild != nullptr){
						std::cout << "next:"<< leftChild->data_ << std::endl;
					...
					if (isRoot){
						...
						parent->right_ = nodeToDelete->right_;
					}
					delete nodeToDelete;
				}
					...
					if (isRoot){
						...
						parent->left_ = nodeToDelete->left_;
					}
					delete nodeToDelete;
				}
				else {
					Node* nextParent = nodeToDelete;
					Node* next = nodeToDelete->right_;
					while (next->left_ != nullptr) {
						nextParent = next;
						next = next->left_;
					}
					if (nextParent != nodeToDelete){
						nextParent->left_ = next->right_;
					}
					else{
						nextParent->right_ = next->right_;
					}
					nodeToDelete->data_ = next->data_;
				}
			}
		}
	}
